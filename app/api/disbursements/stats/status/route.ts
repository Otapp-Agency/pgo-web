...
/**
 * GET /api/disbursements/stats/status
 * Get disbursement statistics grouped by status
 */
    ...
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({
                message: response.statusText || 'Failed to fetch status stats',
            }));

            return NextResponse.json(
                { error: errorData.message || errorData.error || 'Failed to fetch status stats' },
