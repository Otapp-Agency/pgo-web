...
/**
 * POST /api/disbursements/search
 * Search disbursements using the search endpoint with advanced criteria
 */
export async function POST(request: NextRequest) {
    try {
        ...
        console.log('[Disbursements Search API] Searching:', url);
        console.log('[Disbursements Search API] Criteria:', JSON.stringify(requestBody, null, 2).substring(0, 500));
        ...
        if (!response.ok) {
            ...
            console.error('[Disbursements Search API] Error:', response.status, errorData);
            return NextResponse.json(
        ...
        if (data.data && Array.isArray(data.data)) {
            console.log('[Disbursements Search API] Received:', data.data.length, 'disbursements');
        }
        ...
        if (data.data && Array.isArray(data.data)) {
            ...
            console.error('[Disbursements Search API] Unexpected response format:', data);
            return NextResponse.json(
        ...
        console.error('[Disbursements Search API] Error:', error);
        return NextResponse.json(
