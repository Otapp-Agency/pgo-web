...
/**
 * POST /api/payment-gateways/[id]/deactivate - Deactivate a payment gateway
 * FR-PGO-003: Enable/Disable Payment Gateway
 * Note: Uses the gateway's uid (passed as id param) to call backend deactivate endpoint
 */
export async function POST(
    ...
    try {
        ...
        if (!response.ok) {
            console.error('Backend deactivate payment gateway error:', {
                status: response.status,
                data,
                uid,
            });
            return NextResponse.json(
                { error: data.message || data.error || 'Failed to deactivate payment gateway' },
                { status: response.status }
            );
        }
